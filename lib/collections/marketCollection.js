...
MarketSchema = new SimpleSchema({
  ...
  mktAddrType: {
    ...
    type: String,
    allowedValues:
    ["Abakpa Market","Kpirikpiri Market","Ugbodo Market","Eke Imoha Market"],
    autoform: {
...
Market.attachSchema(MarketSchema);

Meteor.methods({
  /**
   * Invoked by autoform to add a new Market record.
   * @param doc The Market document.
   */
  addMarket: function(doc) {
    if(this.userId){
    console.log("Adding", doc);
    //check(doc, Market.simpleSchema());
    doc.createdOn = new Date();
    doc.userId = this.userId;
    console.log("userID: ", this.userId);
    return Market.insert(doc, function(err, docID) {console.log("DocID: ", docID);});
  }
  else{
    return null;
  }
  ...
  /**
   *
   * Invoked by autoform to update a Market record.
   * @param doc The Market document.
   * @param docID It's ID.
   */
  updateMarket: function(doc, docID) {
    if(Meteor.user()){
    console.log("Updating", doc);
    //check(doc, Market.simpleSchema());
    Market.update({_id: docID}, doc);
    }
    else{
      return null;
    }
